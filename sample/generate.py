...
def main(args=None):
    ...
    print(f"saving results npy file to [{npy_path}]")
    np.save(npy_path,
...
def save_multiple_samples(out_path, file_templates, animations, fps, max_frames, no_dir=False):
    ...
    for sample_i in range(0, n_samples, num_samples_in_out_file):
        ...
        # animations_group: shape (n_rows, n_reps) where n_rows = number of samples in this batch.
        animations_group = animations[sample_i:last_sample_i]
        n_rows, n_cols = animations_group.shape  # n_cols == number of repetitions
        
        # assume each element is a numpy array with shape (n_frames, H, W, 3)
        n_frames, H, W, _ = animations_group[0, 0].shape
        
        # create a figure with grid of subplots
        fig, axs = plt.subplots(n_rows, n_cols, figsize=(n_cols*3, n_rows*3))
        # 만약 2D array가 아니라 1D인 경우 처리
        ...
        # 초기 화면: 각 서브플롯에 첫 프레임을 imshow로 보여줌
        ims = [[axs[i][j].imshow(animations_group[i, j][0]) for j in range(n_cols)] for i in range(n_rows)]
        ...
        def update(frame_idx):
            for i in range(n_rows):
                for j in range(n_cols):
                    # 업데이트할 이미지: animations_group[i, j]의 frame_idx번째 이미지
                    ims[i][j].set_data(animations_group[i, j][min(frame_idx, n_frames-1)])
            return sum(ims, [])  # flatten list for FuncAnimation
        
        anim = FuncAnimation(fig, update, frames=max_frames, interval=1000/fps, blit=True)
        
        writer = FFMpegWriter(fps=fps)
        anim.save(all_sample_save_path, writer=writer)
        plt.close(fig)
